...
client.on("messageCreate", async (message) => {
  ...
  if (message.content.toLowerCase().startsWith("-caduceus")) {
    ...
    const CAD_DESC_EMOJI = [
      ...
      // ⚠️ KHÔNG thêm emoji cho "Stamina" — Fragaria 14/08: `<:Fix_Stamina:…>`
      // KHÔNG TỒN TẠI, Discord in ra nguyên chuỗi `:Fix_Stamina:` giữa câu.
      // Muốn thêm lại thì phải có ID emoji THẬT đã upload lên server.
    ];
  ...
  if (/^-skill(\s|$)/i.test(message.content)) {
    ...
    // ── Dice Up / Dice Down (Fragaria 14/08) ─────────────────────────────────
    // Cú pháp: `-skill durandal diceup 3` · `-skill durandal dicedown 2`
    // Gỡ khỏi input TRƯỚC mọi bước phân tích khác, nếu không thì con số sẽ bị
    ...
    if (multiNames) {
      ...
      message.reply({ content: modNote || undefined, embeds });
      return;
    }
    ...
    try {
      await withLock(userId, async () => {
        ...
        if (kv["injury"]) {
          ...
          // ❗ Chấn thương VĨNH VIỄN — chặn ở đường lệnh text nữa (đường nút bấm
          // chặn ở interaction-handlers.js). Sót một đường là người chơi vẫn chữa được.
          if (isPermanentInjury(injuryList[idx])) {
            throw new Error(`**${injuryList[idx]}** là chấn thương VĨNH VIỄN — không chữa được bằng Ahn, K-Corp Ampule hay bất kỳ hình thức nào. Chỉ GM mới gỡ được.`);
          }
...
if (message.content.startsWith("-gacha")) {
  ...
  if (message.content.startsWith("-encounter")) {
    ...
    if (sub === "status") {
      ...
      await message.reply({
        content: bgmPlayable
          ? `🎵 **${bgmName}** — ${(describeEncounterBgm(encounter)?.label ?? "BGM trận này")}`
